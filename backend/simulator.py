...
class Simulator: 
    ...
    def updateOPCUA(self) -> None:
        try:
            ...
            logging.info("OPCUA Client started")
            self.ouaClient.changeParam("Runtime", int(self.simulatedMachine.getMachineRuntime()))
            self.ouaClient.changeParam("Coolant_Level", int(self.simulatedMachine.getCoolantLevelPercent()))
            self.ouaClient.changeParam("Power_Consumption", int(self.simulatedMachine.getPowerConsumptionKWH())/1000) # in kWh
            self.ouaClient.changeParam("Power_Laser", int(self.simulatedMachine.getLaserModulePowerWeardown()))
            self.ouaClient.changeParam("Idle_Time", int(self.simulatedMachine.getIdleTime()))
            self.ouaClient.getParam()
        except:
            self.ouaClient.client.disconnect()
    ...
    def updateModbus(self) -> None:
        try:
            ...
            logging.info("ModbusTCP Client started")
            self.modbusClient.writeSingleRegister(0, int(self.simulatedMachine.getMachineRuntime()))
            self.modbusClient.writeSingleRegister(1, int(self.simulatedMachine.getCoolantLevelPercent()))
            self.modbusClient.writeSingleRegister(2, int(self.simulatedMachine.getPowerConsumptionKWH())/1000) # in kWh
            self.modbusClient.writeSingleRegister(3, int(self.simulatedMachine.getLaserModulePowerWeardown()))
            self.modbusClient.writeSingleRegister(4, int(self.simulatedMachine.getIdleTime()))
